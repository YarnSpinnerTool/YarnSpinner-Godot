    ...
    public class Actions : ICommandDispatcher
    {
        internal class CommandRegistration : ICommand
        {
            ...
            public enum CommandType
            {
                ...
                /// </summary>
                /// <remarks>
                /// Code that invokes this command can await the ask.
                /// </remarks>
                IsCoroutine,
                /// <summary>
            ...
            internal CommandDispatchResult Invoke(DialogueRunner dispatcher, List<string> parameters, out Task commandTask)
            {
                ...
                if (DynamicallyFindsTarget)
                {
                    ...
                    // Node here is referring to Godot nodes, not YarnSpinner dialogue nodes.
                    var nodeName = parameters[0];

                    parameters.RemoveAt(0);

                    var targetNode = FindActionTargetNode(nodeName);
                    ...
                    if (targetNode.GetType() == DeclaringType)
                    {
                        commandTask = default;
                        return new CommandDispatchResult
                        {
        ...
        public void RegisterActions() {
            foreach (var registrationFunction in ActionRegistrationMethods) {
                // TODO - when action generation is working, implement the command dispatcher interface 
                // on dialogue runner and enable this line
        ...
        public void AddCommandHandler<T1, T2, T3, T4, T5, T6, T7>(string commandName, Func<T1, T2, T3, T4, T5, T6, T7, Task> handler) => AddCommandHandler(commandName, (Delegate)handler);
        public void AddCommandHandler<T1, T2, T3, T4, T5, T6, T7, T8>(string commandName, Func<T1, T2, T3, T4, T5, T6, T7, T8, Task> handler) => AddCommandHandler(commandName, (Delegate)handler);
        public void AddCommandHandler<T1, T2, T3, T4, T5, T6, T7, T8, T9>(string commandName, Func<T1, T2, T3, T4, T5, T6, T7, T8, T9, Task> handler) => AddCommandHandler(commandName, (Delegate)handler);
        public void AddCommandHandler<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10>(string commandName, Func<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, Task> handler) => AddCommandHandler(commandName, (Delegate)handler);
        // GYB1 END
        ...
        CommandDispatchResult ICommandDispatcher.DispatchCommand(string command, out Task commandCoroutine)
        {
        ...
        /// <summary>
        /// Search the scene tree for nodes with the specified name via Node.FindChild.
        /// Similar to GameObject.Find in Unity -  rather slow since it searches the
        /// entire tree.
        /// </summary>
        /// <param name="nodeName"></param>
        /// <returns></returns>
        public static Godot.Node FindActionTargetNode(string nodeName)
        {
            return ((SceneTree)Engine.GetMainLoop()).Root.FindChild(nodeName, true, false);
        }
        ...
        private static Converter CreateConverter(ParameterInfo parameter, int index)
        {
            ...
            // find the Godot.Node.
            if (typeof(Godot.Node).IsAssignableFrom(targetType))
            {
                return FindActionTargetNode;
            }
            ...
            if (typeof(Component).IsAssignableFrom(targetType))
            {
                return arg =>
                {
                    Godot.Node node = FindActionTargetNode(arg);
                    if (node == null)
                    {
                        return null;
                    }
                    ...
                    {
                        GD.PushWarning($"Found the node name {arg}, but it was not of the correct script type {targetType.FullName}");
                        return null;
                    }
                    // in Godot, the Node IS the script - it does not have multiple component scripts on one node.
                    return Convert.ChangeType(node, targetType);
                };
            }
