...
namespace YarnSpinnerGodot.Editor
{
    public static class ActionSourceCodeGenerator
    {
            ...
            get
            {
                ...
                else
                {
                    ...
                    catch (System.Exception e)
                    {
                        ...
                        GD.PushWarning($"Can't check to see if {existingFile} is a valid action registration script, using {DefaultOutputFilePath} instead: {e}");
                        return DefaultOutputFilePath;
                    }
                }
            }
        ...
        {
            var analysis = new YarnSpinnerGodot.Editor.ActionAnalyser.Analyser("res://");
            try
            {
                var actions = analysis.GetActions();
                var source = YarnSpinnerGodot.Editor.ActionAnalyser.Analyser.GenerateRegistrationFileSource(actions);

                var path = GeneratedSourcePath;

                System.IO.File.WriteAllText(path, source);

                GD.Print($"Generated Yarn command and function registration code at {path}");
            }
            catch (YarnSpinnerGodot.Editor.ActionAnalyser.AnalyserException e)
            {
                GD.PushError($"Error generating source code: " + e.InnerException.ToString());
            }
        }
    }
}
