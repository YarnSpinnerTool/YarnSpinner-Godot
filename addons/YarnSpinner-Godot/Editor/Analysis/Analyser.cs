...
namespace YarnSpinnerGodot.Editor.ActionAnalyser
{
    ...
    public class Analyser
    {
        ...
        public static MethodDeclarationSyntax GenerateRegistrationMethod(IEnumerable<Action> actions)
        {
            ...
            var registrationMethodBody = SyntaxFactory.Block().WithStatements((SyntaxList<StatementSyntax>)SyntaxFactory.List(registrationStatements));
        ...
        {
            var globalized = ProjectSettings.GlobalizePath(sourcePath);
            if (Directory.Exists(globalized))
            {
                return System.IO.Directory.EnumerateFiles(globalized, "*.cs", SearchOption.AllDirectories);
            }
